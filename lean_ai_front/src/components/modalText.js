...
// ModalText 컴포넌트: 긴 글이 있는 모달 창을 렌더링하는 컴포넌트
const ModalText = ({ show, onClose, title, children }) => {
    ...
    return (
        ...
        <div className="fixed inset-0 bg-gray-600 bg-opacity-50 flex justify-center items-center font-sans">
            {/* 모달의 콘텐츠를 담는 컨테이너 */}
            <div className="bg-white rounded-lg shadow-lg w-1/3 p-4 relative max-h-[80vh] overflow-hidden">
                {/* 모달 닫기 버튼 */}
                <button 
                ...
                {/* 모달의 자식 요소들을 표시하는 영역 */}
                <div className="overflow-scroll max-h-[calc(80vh-4rem)] font-normal text-base">
                    {children}  {/* 모달의 본문 내용 */}
                </div>
            </div>
        </div>
