...
const ComplaintDetailModal = ({ show, onClose, complaint, onStatusChange }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center px-2 z-50">
      <div className="relative bg-white rounded-lg shadow-lg w-full md:w-9/12 p-6">
        <div className="flex justify-between items-center mt-5 md:mt-0 mb-4">
          ...
          <div className="grid gap-3 py-2">
            ...
            <div className="grid grid-cols-4 items-start gap-4">
              <CornerDownRight
                ...
                onClick={() => setShowAnswer((prev) => !prev)} // 상태를 토글
              />
              {showAnswer && (
            ...
            <div className="mt-3 relative">
              ...
              <div className="bg-indigo-50 p-2 rounded-r-lg border-y border-r border-indigo-200">
                <div className="flex flex-col md:flex-row items-center justify-start ">
                  <div className='flex flex-row  items-center justify-start'>
                    <AlertCircle className="h-5 w-5 text-indigo-600 mr-3" />
                    <span className="text-gray-700 cursor-pointer mr-6 whitespace-nowrap" style={{ fontFamily: "NanumSquareBold" }} onClick={handleTransferModal}>
                      해당 민원을 담당하는 부서가 아니라면?
                    </span>
                  </div>
                  <button
                    className="text-indigo-500 hover:text-indigo-600 transition-colors hidden md:block"
      ...
      <TransferDepartmentModal
        ...
        depart={department}
        onTransfer={() => {
          handleTransfer(); // TransferDepartmentModal 닫기
          handleClose(); // ComplaintDetailModal 닫기
        }}
