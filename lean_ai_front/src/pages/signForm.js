...
const Signup = () => {
    ...
    const handleMarketingAgree = () => {
        setMarketingAgreed(true); // 마케팅 동의 상태 업데이트
        setMarketingAccepted(true); // 마케팅 체크박스 상태 업데이트
        setShowMarketingModal(false); // 마케팅 모달 닫기
    };
    ...
    const handleInputChange = (e) => {
        ...
        setFormData({
            ...formData, // 기존의 폼 데이터를 유지하면서
            [name]: value, // 변경된 필드의 값 업데이트
        });
    };
    ...
    **부칙**
    ...
    const handleSignup = async () => {
        ...
        try {
            const response = await fetch('/api/signup', {
                ...
                body: JSON.stringify({
                    ...formData, // 폼 데이터를 JSON 형식으로 전송
                    marketingAccepted, // 마케팅 동의 여부 포함
                }),
            });
            ...
            if (data.success) {
                ...
                setErrorMessage(data.message || '회원가입에 실패했습니다.'); // 실패 메시지 설정
            }
        } catch (error) {
            console.error('회원가입 오류:', error);
            setErrorMessage('회원가입 요청 중 오류가 발생했습니다.'); // 오류 메시지 설정
        }
    };
    ...
    return (
        <div className="bg-blue-100 flex flex-col items-center min-h-screen overflow-y-auto relative w-full">
            ...
            <div className="bg-white p-5 rounded-lg shadow-lg flex flex-col items-center text-center mt-2 mb-4 py-1.5 w-1/3 text-sm font-bold mb-2">
                ...
                <div className="space-y-4">
                    ...
                    <div className="flex items-center space-x-2 mt-4">
                        <input
                            type="checkbox"
                            checked={termsAccepted} // 이용약관 동의 상태에 따라 체크박스 상태 결정
                    ...
                    <div className="flex items-center space-x-2">
                        <input
                            type="checkbox"
                            checked={marketingAccepted} // 마케팅 동의 상태에 따라 체크박스 상태 결정
                    ...
                    <button
                        ...
                        onClick={handleSignup} // 클릭 시 회원가입 함수 호출
                    >
                        회원가입
                    </button>
                </div>
            ...
            {/* 이용약관 모달 */}
            <Modal
                show={showTermsModal} // 이용약관 모달 표시 여부에 따라 모달 표시
            ...
            {/* 마케팅 활용 동의 모달 */}
            <Modal
                show={showMarketingModal} // 마케팅 모달 표시 여부에 따라 모달 표시
                onClose={() => setShowMarketingModal(false)} // 모달 닫기 함수
