...
const NewsCard = ({ category, title, date, content, onClick }) => (
  ...
  >
    ...
    <div className="p-6">
      <h3 className="text-black text-xl font-bold mb-2 h-10 md:h-16 truncate">
        {title}
      </h3>
...
const Newsroom = () => {
  ...
  return (
    <div className="bg-violet-50 min-h-screen p-8 text-indigo-600 flex items-center justify-center">
      <div
        className="bg-white rounded-lg py-12 px-8 shadow-xl"
        style={{ minWidth: "95%" }}
      >
        {/* 헤더 및 뒤로가기 버튼 */}
        <div className="flex flex-row">
          ...
          <h1
            className="text-3xl md:text-4xl font-bold text-left text-indigo-600 mb-4"
        ...
        >
          {filters.map((filter) => (
            <button
              key={filter}
              className={`px-4 md:px-6 py-2 rounded-full transition duration-300 whitespace-nowrap ${
                activeFilter === filter
