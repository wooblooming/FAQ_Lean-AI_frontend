...
const API_DOMAIN = process.env.NEXT_PUBLIC_API_DOMAIN; // API 도메인 등 환경설정 정보
...
const StoreIntroductionPublic = () => {
  ...
  return (
    <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50">
      <div
        className="bg-white rounded-lg shadow-lg relative font-sans overflow-y-auto"
      ...
      >
        ...
        <div className="flex flex-col my-3 pl-4">
          {publicData.public_name && (
            <p id="publicName" className="font-bold text-2xl">
              {publicData.public_name}
            </p> // 매장 이름
          )}
        </div>
        ...
        <div className="tabs flex justify-around border-b-2 font-medium border-gray-300">
          ...
          <button
            className={`p-2 w-1/3 ${
              activeTab === "complaint"
                ? "text-indigo-600 text-xl font-bold border-b-4 border-indigo-500 text-center"
                : ""
            }`}
            style={{
              fontFamily:
        ...
        <div
          {...handlers}
          className="tab-content p-4 font-sans mt-3"
