...
const StoreIntroduction = () => {
  return (
    <div className="bg-white flex flex-col items-center min-h-screen overflow-y-auto relative">     {/* className은 css 설정 내용*/}
      <div className="bg-white rounded-lg p-8 w-full max-w-md text-center mb-2">
        <img src={`${process.env.PUBLIC_URL}/store.png`} alt="Store" className="mx-auto mb-4 w-64 h-52 object-contain" />
      ...
      <Link to="/chatbot"> {/* Link : 특정 주소로 넘어갈 수 있게 함 */}
        <img src={`${process.env.PUBLIC_URL}/chatbot.png`} alt="Chatbot" className="fixed bottom-4 right-4 w-24 h-24 object-cover" />
      </Link>
    </div>
  );
};
